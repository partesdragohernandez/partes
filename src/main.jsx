...
function fromApi(r){return {...r,name:r.name||"",surname:r.surname||"",dni:r.dni||"",phone:r.phone||"",address:r.address||"",insurer:r.insurer||"",claimNo:r.claim_no||"",time:r.time||"",description:r.description||"",observations:r.observations||"",hasDamage:r.has_damage||"",damageWhere:r.damage_where||"",trades:r.trades||"",sqm:r.sqm||"",injuredPhone:r.injured_phone||"",housingNo:r.housing_no||"",injuredDamage:r.injured_damage||"",signerDni:r.signer_dni||"",status:r.status||"BORRADOR",createdAt:r.created_at?new Date(r.created_at).getTime():Date.now(),updatedAt:r.updated_at?new Date(r.updated_at).getTime():Date.now(),fecha:r.created_at?String(r.created_at).slice(0,10):nowDate(),photos:r.photos||[],signature:r.signature||""}}
