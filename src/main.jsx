...
<div class="empty" id="empty"><div class="empty-icon"></div><h1>Gesti&oacute;n de partes de siniestro</h1><p>Crea un parte, a&ntilde;ade los datos, las fotograf&iacute;as y la firma.</p><button class="btn primary" id="emptyNew">Crear primer siniestro</button></div>
...
<div class="bottom-actions"><button class="btn secondary" id="downloadPhotosBtn">Descargar fotograf&iacute;as</button><button class="btn secondary" id="printBtn">Imprimir / Guardar PDF</button><button class="btn danger" id="deleteBtn">Eliminar siniestro</button></div>
</div>
</section></main>
...
if(appMode==="admin"){
  ...
  const today=cs.filter(c=>c.fecha===nowDate());
  ...
  f=cs.filter(c=>c.fecha!==nowDate() && (!d||c.fecha===d) && (!t||(c.direccion+" "+c.nombre+" "+c.apellido+" "+c.numParte+" "+c.aseguradora).toLowerCase().includes(t)));
}
...
$("recordsList").innerHTML=f.map(c=>`<div class="record ${c.id===currentId?"active":""}" data-id="${c.id}"><strong>${esc(c.direccion||"Sin direcci&oacute;n")}</strong><small>${esc(c.fecha||"")} ${esc(c.hora||"")} - ${esc((c.nombre+" "+c.apellido).trim()||"Sin asegurado")}</small><div class="status">${esc(c.status)}${appMode==="worker"&&c.fecha===nowDate()?" - EDITABLE HOY":""}</div></div>`).join("");
...
async function addFiles(files){const list=[...(files||[])];if(!list.length)return;let n=0;for(const f of list){if(!f.type.startsWith("image/"))continue;try{let data=await new Promise((res,rej)=>{let r=new FileReader();r.onload=()=>res(r.result);r.onerror=()=>rej(new Error("No se pudo leer la foto"));r.readAsDataURL(f)});photos.push({name:f.name,type:f.type,data,addedAt:Date.now()});n++}catch(e){console.error(e)}}renderPhotos();$("photos").value="";if(n)toast(`${n} fotograf&iacute;a${n===1?"":"s"} anadida${n===1?"":"s"}`);else toast("No se pudo anadir la fotograf&iacute;a")}
...
$("deleteBtn").onclick=async()=>{if(currentId&&confirm("Eliminar este siniestro y sus fotograf&iacute;as?")){await delCase(currentId);currentId=null;$("editor").classList.add("hidden");$("empty").classList.remove("hidden");renderList();toast("Siniestro eliminado")}};
