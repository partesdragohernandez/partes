...
export function setupAccount({ onUser, onWorkersChanged, notify }) {
  ...
  const show = user => {
    ...
    password.querySelector("#passwordRequirement").textContent = admin ? "Nueva contraseña (mínimo 15 caracteres: mayúscula, minúscula, número y símbolo)" : "Nueva contraseña (mínimo 6 caracteres)";
    password.querySelectorAll("[name=newPassword],[name=repeat]").forEach(input => input.minLength = admin ? 15 : 6);
  ...
  dialog.innerHTML = '<h2>Trabajadores</h2><form id="workerForm"><label>Nombre visible<input name="displayName" required maxlength="200"></label><label>Usuario<input name="username" required pattern="[a-zA-Z0-9._-]{3,80}" autocomplete="off"></label><label>Contraseña temporal (mínimo 6 caracteres)<input name="password" type="password" autocomplete="new-password" required minlength="6" maxlength="128"></label><button class="btn secondary" type="submit">Crear trabajador</button></form><p role="alert" id="workersError"></p><div id="workersList"></div><button class="btn ghost" id="closeWorkers" type="button">Cerrar</button>';
  ...
  const refreshWorkers = async () => {
    ...
    for (const user of users) {
      ...
      resetForm.innerHTML='<label>Nueva contraseña temporal (6–128 caracteres)<input type="password" autocomplete="new-password" minlength="6" maxlength="128" required></label><button class="btn secondary" type="submit">Restablecer contraseña</button>';
