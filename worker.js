    ...
    try {
      if (request.method === "GET" && url.pathname === "/api/cases") {
        const r = await env.DB.prepare(
          "SELECT id,name,surname,address,claim_no,status,created_at,updated_at FROM cases WHERE user_id=? ORDER BY updated_at DESC"
        ).bind(uid).all();
      ...
      if (request.method === "POST" && url.pathname === "/api/cases") {
        ...
        const keys = ["name","surname","dni","phone","address","insurer","claimNo","time","description","observations","hasDamage","damageWhere","trades","sqm","injuredPhone","housingNo","injuredDamage","signerDni","status"];
