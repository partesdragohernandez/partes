...
try{
 ...
 const temporary=randomBytes(3).toString('hex');for(const [name,value] of Object.entries({displayName:'Trabajador ficticio',username:'nuevo',password:temporary}))await page.locator(`#workerForm [name=${name}]`).fill(value);await page.locator('#workerForm button').click();await page.waitForFunction(()=>document.querySelector('#workersList').textContent.includes('Trabajador ficticio'));
 assert.equal(await page.locator('#workerForm [name=email]').count(),0);
